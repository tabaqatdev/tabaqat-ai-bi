...
export default function DatabricksProperties(props: Props) {
  ...
  const initialDatabricksTypeRef = useRef<DATABRICKS_AUTH_METHOD | null>(null);
  ...
  useEffect(() => {
    if (
      isEditMode &&
      databricksType &&
      initialDatabricksTypeRef.current === null
    ) {
      initialDatabricksTypeRef.current = databricksType;
    }
  }, [isEditMode, databricksType]);
